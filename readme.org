...
** Fork notes
This fork adds an `alt+tab` macro when you swap between the primary and secondary layout,

thus enabling you to use a hardcoded dvorak/colemak layout while still using qwerty for the second language on your computer
...
Used to select an alternative ZMK branch or tag for building, and to merge branches into ZMK at build time.

Branches are specified in the form ~<user>/<repo>/<branch-or-tag>~.  The default pinned ZMK version is ~zmkfirmware/zmk/v0.3.0~.
...
Miryoku uses the official ZMK mouse emulation behaviors ~&mkp~, ~&mmv~, and ~&msc~, with its existing movement and scroll speed values.
...
Layer switcher keys use a double tap guard by default. To disable the guard and switch layers with a single tap, add ~#define MIRYOKU_KLUDGE_NO_DOUBLETAPGUARD~ to the [[#config-file][config file]]. For [[#workflow-builds][workflow builds]], use ~#define MIRYOKU_KLUDGE_NO_DOUBLETAPGUARD~ with the ~custom_config~ option.
...
For [[#workflow-builds][workflow builds]] using the [[#build-inputs][Build Inputs]] workflow, use ~xmk~ with the ~shield~ option, the appropriate board with the ~board~ option, and ~petejohanson/zmk/shell/tap-command~ with the ~branches~ option.  Alternatively, use ~zmkfirmware/zmk/v0.3.0 petejohanson/zmk/shell/tap-command~ to attempt an automatic [[#branches][merge]] of the branch into the pinned ZMK release.
...
For [[#workflow-builds][workflow builds]] using the [[#build-inputs][Build Inputs]] workflow, use ~native_posix_64~ with the ~board~ option, ~#define MIRYOKU_KLUDGE_TAPDELAY~ with the ~custom_config~ option, and ~petejohanson/zmk/shell/tap-command~ with the ~branches~ option.  Alternatively, use ~zmkfirmware/zmk/v0.3.0 petejohanson/zmk/shell/tap-command~ to attempt an automatic [[#branches][merge]] of the branch into the pinned ZMK release.
